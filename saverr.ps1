...
$checkBox_debug.AllowDrop        = $false
...
function clearStatusSave {
    ...
    if (Test-Path .\saverrSettings.xml) {
        ...
        if ($debug) {
            Add-Member -InputObject $settings -MemberType NoteProperty -Name 'logging' -Value $true -force
            $settings | Export-Clixml .\saverrSettings.xml
        }
        else {
            Add-Member -InputObject $settings -MemberType NoteProperty -Name 'logging' -Value $false -force
            $settings | Export-Clixml .\saverrSettings.xml
        }
    }
    else {
...
function clearDLStatus {
    ...
    if ($label_DLTitle.Text -match "^Download Completed|^Download Failed|^Download Cancelled|^There was an error") {
            $label_DLTitle.Text = ""
    }
...
#$dl_url =  $url + "$($xml.MediaContainer.Video.Media.Part.key)" + $tokenpath
...
$button_download.Add_Click({
    ...
    Try {
        if ((Get-BitsTransfer).JobState -ne "Suspended") {
            ...
            if ($type -eq "artist") {
                ...
                # if all tracks and all albums is selected #test with: $script:seasons = $xmlSeason.MediaContainer.directory | select title,key,index,year
        ...
        if ($count -ge 30) {
            ...
            $label_DLTitle.ForeColor = "#ff0000"
            $label_DLTitle.Text = "Download Failed! Timeout reached. Check network/server maybe?"
            $button_download.Enabled = $true
            $label_DLProgress.Text = ""
        }
        ...
        if ($status -eq "success") {
            ...
            if ($script:cancelLoop -eq $false -and $script:pauseLoop -eq $false) {
                ...
                $label_DLTitle.Text = "Download Completed in: $((Get-Date).Subtract($start_time).Minutes) minutes"
                $button_download.Enabled = $true
                $button_cancel.Visible = $false
                $button_cancel.Enabled = $false
                $checkBoxButton_pause.Visible = $false
    ...
    Catch {
        ...
        $label_DLTitle.ForeColor = "#ff0000"
        $label_DLProgress.Text = ""
        $label_DLTitle.Text = "There was an error with the download. Please try again."
        $progressBar.Visible = $false
        $progressBar.Value = 0
        $button_download.Enabled = $true
        $button_cancel.Visible = $false
...
$textBox_search.Add_KeyUp({
 ...
 $checked_type ={
             if ($RadioButton_movie.Checked){
                   #[System.Windows.Forms.MessageBox]::Show("You selected movies")
                   clearMediaInfo
                   clearDLStatus
                   $textBox_search.Text = ""
                   $label_seasons.Text = ""
            ...
            if ($RadioButton_tv.Checked){
                   #[System.Windows.Forms.MessageBox]::Show("You Selected tv")
                   clearMediaInfo
                   clearDLStatus
                   $textBox_search.Text = ""
            ...
            elseif ($radiobutton_music.Checked) {
                   #[System.Windows.Forms.MessageBox]::Show("You Selected music")
                   clearMediaInfo
                   clearDLStatus
                   $textBox_search.Text = ""
...
# this fires after the window is minimized though
$form.add_Resize({
    #[System.Windows.Forms.MessageBox]::Show("You Selected tv")
    #$script:pauseLoop = $true
    #$script:CancelLoop = $true
    #Suspend-BitsTransfer $myjob
})
